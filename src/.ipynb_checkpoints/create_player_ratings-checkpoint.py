import pandas as pd
import numpy as np
from src.get_data import load_boxscore_schedule
import re
...
def get_stint_pts_data(game_id: str):
    ...
    df_schedule, df_boxscores = load_boxscore_schedule()

    # start_ end_period ensures that it is only regulation
    ...
    # all substitutions!
    subs = data.loc[bool_subs, ['PLAYER1_ID', 'PLAYER2_ID', 'stint', 'stint_marker']]

    # get the games boxscore
    game = df_boxscores[df_boxscores['GAME_ID'] == game_id]
    ...
    tmp_ht = df_schedule[df_schedule['GAME_ID'] == 22001074]
